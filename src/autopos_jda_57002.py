from autopos_jda_common import generate_data_file, prepare_data_b2s, query_data
from common import sftp
from datetime import datetime, timedelta
import os, sys, traceback


def main():
  try:
    store = '57002'
    bu = 'b2s'
    str_date = sys.argv[1] if len(sys.argv) > 1 else datetime.now().strftime('%Y%m%d')
    dir_path = os.path.dirname(os.path.realpath(__file__))
    parent_path = os.path.abspath(os.path.join(dir_path, os.pardir))
    target_path = os.path.join(parent_path, 'output/autopos/jda/{}'.format(
        bu.lower()), str_date)
    if not os.path.exists(target_path):
      os.makedirs(target_path)

    files = generate_data_file(target_path, store, prepare_data_b2s(query_data(store, str_date)))
    destination = 'incoming/jda/{}'.format(bu.lower())
    sftp('autopos.cds-uat', target_path, destination, files)
  except Exception as e:
    print('[AutoPOS] - JDA Error: %s' % str(e))
    traceback.print_tb(e.__traceback__)


if __name__ == '__main__':
  main()
