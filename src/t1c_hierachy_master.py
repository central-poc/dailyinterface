...
with connect_cds_db() as conn:
  ...
  query = '''
    SELECT
        ...
        'CDS' AS JDA_BUCODE
    FROM
        DBCDSContent.dbo.TBDepartment level0
    LEFT OUTER JOIN
        DBCDSContent.dbo.TBDepartment level1
    ON
        level0.DepartmentId = level1.ParentId
    LEFT OUTER JOIN
        DBCDSContent.dbo.TBDepartment level2
    ON
        level1.DepartmentId = level2.ParentId
    LEFT OUTER JOIN
        DBCDSContent.dbo.TBDepartment level3
    ON
        level2.DepartmentId = level3.ParentId
    LEFT OUTER JOIN
        DBCDSContent.dbo.TBDepartment level4
    ON
        level3.DepartmentId = level4.ParentId
    ORDER BY
        Level0ID ASC;
    '''
...
with connect_rbs_db() as conn:
  ...
  query = '''
    SELECT
      ...
      'RBS' as JDA_BUCODE
    FROM
      [DBMKPOnline].[dbo].[GlobalCategory] t0
    LEFT JOIN [DBMKPOnline].[dbo].[GlobalCategory] t1
      ON t0.Lft < t1.Lft AND t1.Rgt < t0.Rgt
    LEFT JOIN [DBMKPOnline].[dbo].[GlobalCategory] t2
      ON t1.Lft < t2.Lft AND t2.Rgt < t1.Rgt
    LEFT JOIN [DBMKPOnline].[dbo].[GlobalCategory] t3
      ON t2.Lft < t3.Lft AND t3.Rgt < t2.Rgt
    LEFT JOIN [DBMKPOnline].[dbo].[GlobalCategory] t4
      ON t3.Lft < t4.Lft AND t4.Rgt < t3.Rgt
    ORDER BY Level0ID AS
    '''
...
print("Update CDS, RBS category hierachy in %ds" % execution_time)
