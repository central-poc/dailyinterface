...
with pymssql.connect("mssql.production.thecentral.com", "coreapi", "coreapi",
  ...
  with conn.cursor(as_dict=True) as cursor:
    ...
    LEFT JOIN JDARBS_Dept SubDept on SubDept.IDEPT = pro.JDADept AND SubDept.ISDEPT = pro.JDASubDept AND SubDept.ICLAS = 0 AND SubDept.ISCLAS = 0
    LEFT JOIN JDARBS_Dept Class on Class.IDEPT = pro.JDADept AND Class.ISDEPT = pro.JDASubDept AND Class.ICLAS = pro.ClassCode AND Class.ISCLAS = 0
...
with pymssql.connect("10.17.220.55", "central", "Cen@tral",
  ...
  with conn.cursor(as_dict=True) as cursor:
    start_time = datetime.now()
    sql = """
      if object_id('dbo.temp_siebel_product_inc', 'U') is not null
        drop table dbo.temp_siebel_product_inc
    """
    cursor.execute(sql)
    sql = """
    ...
    """
    cursor.execute(sql)
    sql = "create index idx_siebel_product_inc_pid ON dbo.temp_siebel_product_inc (pid)"
    cursor.execute(sql)
    elapsed_time = (datetime.now() - start_time).seconds
    print("Prepared in {} s.".format(elapsed_time))

    sql = "select count(pid) as c from dbo.temp_siebel_product_inc"
    cursor.execute(sql)
    ...
    for page in range(0, pages):
      start_time = datetime.now()
      sql = """
        select * from dbo.temp_siebel_product_inc
        order by pid
